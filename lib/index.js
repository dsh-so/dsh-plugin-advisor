...
export function apply(ctx, config) {
    ctx.tools.register(defineTool({
        name: 'find_plugin',
        description: 'Search the dsh.so plugin marketplace for DeepSeek Harness plugins that match a need. ' +
            ...
            render: (_args, value) => {
                ...
                if (!matches.length) {
                    return [
                        {
                            type: 'text',
                            text: 'No plugins in the dsh.so plugin marketplace matched that query. Suggest broader terms (e.g. "image", "terminal", "memory").' +
                                footer(config),
                        },
                    ];
                }
